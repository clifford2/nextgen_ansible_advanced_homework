...
# 2021-05-06: check whether this itemID has already been deployed
curl -s -H "X-Auth-Token: $tok" \
              -H "Content-Type: application/json" \
              -X GET \
              "${uri}/api/services" | jq -r ".resources[] | .href" | while read svcURL
do
    svcItem=$(curl -s -H "X-Auth-Token: $tok" \
              -H "Content-Type: application/json" \
              -X GET \
              "${svcURL}" | jq -r ".service_template_id")
    if [ "$itemID" = "$svcItem" ]
    then
        echo "Item '${itemName}' already deployed"
        exit 1
    fi
done || exit
